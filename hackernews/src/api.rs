...
mod tests {
    ...
    #[tokio::test]
    async fn test_top_stories_should_work() {
    ...
    async fn get_comment_by_id_should_work() {
        let stories = get_top_stories(1).await.unwrap();
        let comment = get_comment_by_id(stories[0].kids[0]).await.unwrap();
        assert!(comment.id == stories[0].kids[0]);
    }
}
